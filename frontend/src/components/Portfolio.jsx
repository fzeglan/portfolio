...
function Hero() { /* unchanged for brevity */
    ...
    <section className="hero-gradient pt-24 border-b border-border">
      <div className="mx-auto max-w-7xl px-4 py-16 md:py-24 grid md:grid-cols-2 gap-8 md:gap-12 items-center">
        <div className="order-2 md:order-1">
          ...
          <h1 className="font-extrabold leading-tight text-4xl md:text-5xl lg:text-6xl font-manrope">Olá, eu sou o</h1>
...
function Sobre() { /* unchanged aside from keys already fixed */
    ...
    <section id="sobre" className="section-alt">
      <div className="mx-auto max-w-7xl px-4 py-16 md:py-20 grid md:grid-cols-2 gap-10 items-center">
        <div>
          ...
          <p className="text-muted-foreground text-lg leading-relaxed mt-4">Sou Felipe Zeglan, especialista em automação, tecnologia e programação em Python. Transformo ideias em sistemas que funcionam — de conectores entre plataformas a bots inteligentes e painéis sob medida.</p>
          <div className="flex flex-wrap gap-2 mt-6">
            {chips.map((c, idx) => (<Badge key={`${c}-${idx}`} className="badge-chip">{c}</Badge>))}
          </div>
        </div>
            ...
            return (
              <Card key={s.id} className="card-hover">
                <CardContent className="p-6 flex flex-col h-full">
                  ...
                  <Link to={`/servicos/${s.id}`} className="mt-auto"><Button variant="outline" className="btn-outline w-full">Saiba mais</Button></Link>
                </CardContent>
              </Card>
            );
          })}
...
function Projetos() { /* unchanged except CTA link */
  const onImgError = (e, idx) => { if (e?.currentTarget) e.currentTarget.src = `/images/proj-${idx + 1}.svg`; };
  return (
              ...
              <CardContent className="p-0 flex flex-col h-full">
                ...
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  <div className="flex flex-wrap gap-2 mb-3">
                    {p.tags.map((t, tIdx) => (<Badge key={`${p.id}-${t}-${tIdx}`} className="badge-tag">{t}</Badge>))}
                  </div>
                  {p.repo && (
                    <div className="pt-1">
                      <a href={p.repo} target="_blank" rel="noreferrer"><Button variant="outline" size="sm" className="btn-outline">Ver repositório</Button></a>
                    </div>
                  )}
                </div>
              </CardContent>
...
function Contato() { /* unchanged */
  const [form, setForm] = useState({ nome: "", email: "", mensagem: "" });
  const isValid = useMemo(() => (form.nome.trim().length >= 2 && /.+@.+\..+/.test(form.email) && form.mensagem.trim().length >= 10), [form]);
  const handleSubmit = (e) => { e.preventDefault(); if (!isValid) return; const ok = saveContactSubmission({ ...form }); if (ok) { toast({ title: "Mensagem enviada (mock)", description: "Recebi seus dados localmente. Em breve integro com o backend.", }); setForm({ nome: "", email: "", mensagem: "" }); } else { toast({ title: "Falha ao salvar (mock)", description: "Tente novamente.", }); } };
  return (
    <section id="contato" className="section-cta">
      <div className="mx-auto max-w-4xl px-4 py-16 md:py-20 text-center">
        ...
        <div className="flex items-center justify-center gap-3 mb-10 flex-wrap">
          <a href={CONTACT_LINKS.email} target="_blank" rel="noreferrer"><Button className="btn-primary" variant="default">E-mail</Button></a>
          <a href={CONTACT_LINKS.github} target="_blank" rel="noreferrer"><Button variant="outline" className="btn-outline">GitHub</Button></a>
          <a href={CONTACT_LINKS.linkedin} target="_blank" rel="noreferrer"><Button variant="outline" className="btn-outline">LinkedIn</Button></a>
        </div>
  ...
  return (
    <footer className="border-t border-border bg-background/60">
      <div className="mx-auto max-w-7xl px-4 py-8 flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3"><div className="h-7 w-7 rounded-full bg-gradient-to-br from-purple-500 to-purple-700" /><span className="text-sm text-muted-foreground">© {new Date().getFullYear()} Felipe Zeglan. Todos os direitos reservados.</span></div>
        <nav className="flex items-center gap-5 text-sm"><a href="#" className="nav-link">Home</a><a href="#sobre" className="nav-link">Sobre</a><a href="#projetos" className="nav-link">Projetos</a><a href="#contato" className="nav-link">Contato</a></nav>
      </div>
    </footer>
  );
}
